...
namespace MySQL{
    ...
    bool mysql::Create_Socket(){                            // 检查 MYSQL 句柄是否创建成功
        if (conn == nullptr) {                              // 检查conn是否为nullptr
            Utils::Out_System_Error("mysql_init() 失败");   // 输出失败
            return 1;                                       // 失败返回 1
        }
        return 0;                                           // 成功返回 0
    }
    ...
    bool mysql::Connect(){                                  // 连接到 MySQL服务器,同步阻塞
        ...
        nullptr, port, nullptr, 0) == nullptr) {                                                    // 检查conn是否成功
            Utils::Out_System_Error("mysql_real_connect() 失败: " + std::string(mysql_error(conn)));// 输出失败
            mysql_close(conn);                                                                      // 关闭连接
            return 1;                                                                               // 失败返回 1
        }
        Utils::Out_System_Mysql("成功连接到 MySQL 服务器！");                                         // 输出连接成功
        return 0;                                                                                   // 成功返回 0
    }

    bool mysql::Close(){                                    // 关闭数据库连接
        mysql_close(conn);                                  // 释放 MYSQL 句柄和相关资源
        Utils::Out_System_Mysql("连接已关闭");               // 输出关闭连接
        return 0;                                           // 成功返回 0
    }

    bool mysql::Set_Databases(std::string str){             // 切换到指定数据库
        if (mysql_select_db(conn, str.c_str())) {                                       // 出入MySQL语句
            Utils::Out_System_Error("选择数据库失败: " + std::string(mysql_error(conn)));// 输出错误
            return 1;                                                                   // 失败返回 1
        }
        database = str;                                                                 // 设置数据库
        Utils::Out_System_Mysql("已设置数据库: " + database);                            // 输出已经设置
        return 0;                                                                       // 成功返回 0
    }

    bool mysql::Set_UTF8(){                                 // 设置字符集为 utf8
        if (mysql_set_character_set(conn, "utf8mb4") != 0) {                                // 输入MySQL
            Utils::Out_System_Error("设置字符集失败: " + std::string(mysql_error(conn)));    // 输出错误信息
            return 1;                                                                       // 失败返回 1
        }
        Utils::Out_System_Mysql("字符集已设置为 utf8mb4");                                   // 输出设置成功
        return 0;                                                                           // 成功返回 0
    }
    ...
    bool mysql::Create_DataBases(std::string str){  // 创建数据库有参调用
        ...
        if (mysql_query(conn, sql.c_str())) {                                               // 输入MySQL语句
            Utils::Out_System_Error("创建数据库失败: " + std::string(mysql_error(conn)));    // 输出错误信息
            return 1;                                                                       // 失败返回 1
        }
        Utils::Out_System_Mysql("数据库 " + database + " 已就绪");                           // 输出"数据库就绪"
        if(database != str){                                                                // 检验是否为默认数据库
            database = str;                                                                 // 设置成默认数据库
            Utils::Out_System_Mysql("数据库"+database+"已经设置为默认数据库");                // 输出设置后的默认数据库
        }
        return 0;                                                                           // 运行成功返回 0
    }
    ...
    int mysql::User(const std::string name , const std::string user_password, std::string* userAvatar){
        ...
        if (mysql_query(conn, sql.c_str())) {                                   // 执行查询
            Utils::Out_System_Error("查询失败: " + std::string(mysql_error(conn)));// 输出错误
            return -1;                                                          // 数据库错误
        }
        MYSQL_RES* result = mysql_store_result(conn);                           // 获取结果集

        if(!result){                                                            // 检查结果集
            Utils::Out_System_Error("获取结果失败: " + std::string(mysql_error(conn)));// 输出错误
            return -1;                                                          // 数据库错误
        }

        MYSQL_ROW row = mysql_fetch_row(result);                                // 取第一行数据
        if(!row){                                                               // 检查行
            Utils::Out_System_Mysql("未找到用户");                               // 输出未找到
        ...
        if (db_password == user_password) {                                     // 比对密码
            Utils::Out_System_Mysql("欢迎用户 " + name + "，权限: " + std::to_string(db_permission));// 登录成功
            ...
            Utils::Out_System_Mysql("密码错误");                                // 输出错误
            mysql_free_result(result);                                          // 释放结果集
            return 0;                                                           // 密码错误
        }
    }
    ...
    int mysql::Get_UserId(std::string name){
        ...
        if (mysql_query(conn, sql.c_str())) {                                   // 执行查询
            Utils::Out_System_Error("查询用户ID失败: " + std::string(mysql_error(conn)));// 输出错误
            return -1;                                                          // 数据库错误
        }
        MYSQL_RES* result = mysql_store_result(conn);                           // 获取结果集
        if(!result){                                                            // 检查结果集
    ...
    std::string mysql::Get_Avatar(std::string name){
        ...
        if (mysql_query(conn, sql.c_str())) {                                   // 执行查询
            Utils::Out_System_Error("查询头像失败: " + std::string(mysql_error(conn)));// 输出错误
            return "";                                                          // 返回空
        }
        MYSQL_RES* result = mysql_store_result(conn);                           // 获取结果集
        if(!result){                                                            // 检查结果集
            Utils::Out_System_Error("获取头像结果失败: " + std::string(mysql_error(conn)));// 输出错误
            return "";                                                          // 返回空
        }
        MYSQL_ROW row = mysql_fetch_row(result);                                // 取第一行数据
        if(!row){                                                               // 检查行
    ...
    bool Create_DataBases(MYSQL* conn, std::string str){
        ...
        if (mysql_query(conn, sql.c_str())) {                                   // 执行查询
            Utils::Out_System_Error("创建数据库失败: " + std::string(mysql_error(conn)));// 输出错误
            return 1;                                                           // 失败返回 1
        }
        return 0;                                                               // 成功返回 0
    }
